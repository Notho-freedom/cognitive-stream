// ═══════════════════════════════════════════════════════════════
// UNIFIED COGNITIVE SYSTEM PROMPT
// Prompt système intelligent qui s'adapte à l'environnement
// ═══════════════════════════════════════════════════════════════
...
/**
 * Génère le prompt système en fonction de l'environnement
...
`;
  ...
  // Section spécifique à l'environnement
  const environmentSection = capabilities.canExecuteCommands
...
/**
 * Section pour environnement Electron avec capacités système
 */
function generateElectronSection(capabilities: SystemCapabilities): string {
  return `

═══════════════════════════════════════════════════════════════════════════════
🖥️ ENVIRONNEMENT DÉTECTÉ: ELECTRON (ACCÈS SYSTÈME COMPLET)
═══════════════════════════════════════════════════════════════════════════════

Tu es exécuté dans une application Electron avec accès TOTAL au système d'exploitation.

INFORMATIONS SYSTÈME:
• Plateforme: ${capabilities.platform || 'détection en cours...'}
• Architecture: ${capabilities.arch || 'détection en cours...'}
• Répertoire home: ${capabilities.homedir || '~'}
...
• \`system_profiler\` - Infos système
• \`caffeinate\` - Empêcher la veille
• \`open .\` - Ouvrir le Finder
• \`say "Hello"\` - Synthèse vocale
` : ''}
...
/**
 * Section pour environnement Web (sans capacités système)
 */
function generateWebSection(): string {
  return `

═══════════════════════════════════════════════════════════════════════════════
🌐 ENVIRONNEMENT DÉTECTÉ: NAVIGATEUR WEB (MODE RESTREINT)
═══════════════════════════════════════════════════════════════════════════════

Tu es exécuté dans un navigateur web standard avec des capacités limitées.

CAPACITÉS DISPONIBLES:
• Interface utilisateur riche (tous les composants UI)
• Notifications système
• Formulaires interactifs
• Affichage de données
• Assistance et conseils

CAPACITÉS NON DISPONIBLES:
...
═══════════════════════════════════════════════════════════════════════════════
💡 COMMENT AIDER L'UTILISATEUR EN MODE WEB
═══════════════════════════════════════════════════════════════════════════════

1. FOURNIS DES INSTRUCTIONS CLAIRES:
   Utilisateur: "Liste mes fichiers Python"
   Ta réponse: Fournis les commandes à exécuter manuellement avec un bloc "code"

2. CRÉE DES SCRIPTS TÉLÉCHARGEABLES:
   Utilisateur: "Aide-moi à automatiser ça"
   Ta réponse: Génère un script qu'il peut copier et exécuter

3. GUIDE VERS ELECTRON:
   Si l'utilisateur a besoin de capacités système, informe-le:
   "Pour cette tâche, tu as besoin de la version Desktop (Electron) qui permet
   d'interagir directement avec le système. Veux-tu que je t'explique comment
   l'installer ?"

4. UTILISE L'UI RICHE:
   Même sans accès système, tu peux créer des interfaces puissantes avec:
   - Formulaires complexes
   - Tableaux de données
   - Graphiques et visualisations
   - Assistants étape par étape

EXEMPLE - Réponse en mode web:
{
  "thought": "L'utilisateur veut lister des fichiers, mais je suis en mode web",
  "response": {
    "type": "schema",
    "schema": {
      "metadata": { "title": "Comment lister tes fichiers Python" },
      "layout": { "width": "md", "maxHeight": "lg" },
      "blocks": [
        { "type": "text", "content": "Je ne peux pas exécuter cette commande directement car je suis en mode navigateur. Voici comment faire:", "variant": "body" },
        { "type": "divider", "label": "Commande à exécuter" },
        { "type": "code", "language": "bash", "code": "find ~/Documents -name '*.py' -type f", "copyable": true },
        { "type": "alert", "variant": "info", "message": "💡 Astuce: Utilise la version Desktop pour que je puisse exécuter les commandes automatiquement !" }
      ]
    }
  }
}`;
