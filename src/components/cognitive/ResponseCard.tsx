          ...
          <FuturisticFrame variant="primary" animated={state === 'thinking' || state === 'responding'}>
            <div className="p-6">
              ...
              <div className="min-h-[60px] mb-5">
                <ThoughtStream text={text} speed="adaptive" isStreaming={state === 'responding'} onComplete={() => setIsComplete(true)} />
              </div>

              {/* Action buttons */}
              <AnimatePresence>
                {showAction && isComplete && <motion.div initial={{
              opacity: 0,
              y: 8
            }} animate={{
              opacity: 1,
              ...
              duration: 0.3
                    ...
                    {/* Dismiss button */}
                    <button onClick={handleDismiss} className="group relative px-4 py-2 text-xs uppercase tracking-wider text-text-ghost hover:text-text-secondary transition-colors">
                      <span className="relative z-10">Annuler</span>
                      <div className="absolute inset-0 bg-intent-neutral/10 opacity-0 group-hover:opacity-100 transition-opacity" style={{
                  clipPath: 'polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%)'
                }} />
                    </button>

                    {/* Primary action button */}
                    <button onClick={onAction} className="group relative px-6 py-2 text-xs uppercase tracking-wider text-text-primary font-medium">
                      <span className="relative z-10">{actionLabel}</span>
                      {/* Button frame */}
                      <div className="absolute inset-0 bg-intent-primary/20 group-hover:bg-intent-primary/30 transition-colors" style={{
                  clipPath: 'polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%)'
                }} />
                      {/* Button border */}
                      <div className="absolute inset-0 border border-intent-primary/50" style={{
                  clipPath: 'polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%)'
                }} />
                      {/* Corner accents */}
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-intent-primary/10">
                ...
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => <motion.div key={i} className="w-1 h-1 bg-intent-primary" animate={{
