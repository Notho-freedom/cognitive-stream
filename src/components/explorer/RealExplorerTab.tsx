...
export function RealExplorerTab({ active, initialPath, openToken = 0, onPathChange, onOpenCommandPalette }: Props) {
  ...
  useEffect(() => { onPathChange(explorer.nav.currentFolderId); }, [explorer.nav.currentFolderId, onPathChange]);
  useEffect(() => {
    if (initialPath && openToken > 0) explorer.navigateTo(initialPath);
