...
export function FileGrid({ files, viewMode, selectedItems, clipboardItems, showExtensions, iconSize, renamingId, renamedItems, sortField, sortDirection, onSelect, onOpen, onContextMenu, onClearSelection, onConfirmRename, onCancelRename, onSort, onDropOnFolder }: Props) {
  ...
  const isCut = (id: string) => clipboardItems.includes(id);
  ...
  const dragProps = (file: FileItem) => ({
    ...
    onDragStart: (e: React.DragEvent) => {
      const ids = selectedItems.includes(file.id) && selectedItems.length > 0 ? selectedItems : [file.id];
      e.dataTransfer.effectAllowed = 'copyMove';
      e.dataTransfer.setData(EXPLORER_DND_MIME, JSON.stringify(ids));
    },
    ...
    ...(file.type === 'folder' && {
      ...
      onDragLeave: () => setDropTarget(t => (t === file.id ? null : t)),
                ...
                >
                  <td className="px-3 py-0.5">
                    <div className="flex items-center gap-2">
                      <FileIcon type={file.type} extension={file.extension} name={getName(file)} size={16} />
                      {isRenaming ? (
            ...
            return (
              ...
              >
                {file.thumbnail ? (
                  ...
                  <FileIcon type={file.type} extension={file.extension} name={getName(file)} size={36} />
                )}
            ...
            return (
              ...
              >
                {file.thumbnail ? (
                  ...
                  <FileIcon type={file.type} extension={file.extension} name={getName(file)} size={40} />
                )}
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-auto p-3 relative" onClick={e => { if (e.target === e.currentTarget) onClearSelection(); }}>
      <div className={cn('flex flex-wrap content-start', config.gap)} style={{ ['--icon-scale' as any]: 'var(--icon-scale, 1)' }}>
        {files.map(file => {
          const selected = selectedItems.includes(file.id);
          const isRenaming = renamingId === file.id;
            ...
            >
              {showThumb ? (
                ...
                <div style={{ width: scaledIcon, height: scaledIcon, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <FileIcon type={file.type} extension={file.extension} name={getName(file)} size={config.icon} />
                </div>
              )}
              {isRenaming ? (
