                  ...
                  <div className="flex gap-1">
                    {(['config', 'test'] as const).map(t => (
                      <button
                        ...
                        onClick={() => setTab(t)}
                        className={cn(
                          'px-3 py-1 text-[9px] uppercase tracking-[0.2em] border-b-2 transition-colors',
                          tab === t
                            ? 'text-intent-primary border-intent-primary'
                            : 'text-text-ghost/60 border-transparent hover:text-text-primary',
                        )}
                      >
                ...
                <ScrollArea className="max-h-[66vh] pr-2">
                  {tab === 'config' && (
                    <div className="space-y-4">
                      ...
                      <SettingRow label="Explorateur Windows" description="Remplacer l'explorateur natif"
                      ...
                      <div className="space-y-2 border border-intent-primary/10 p-2">
                        <div className="text-[10px] uppercase tracking-[0.2em] text-text-ghost">État takeover</div>
                        <div className="flex items-center justify-between gap-3 text-[10px] text-text-ghost/70">
                          <span>
                            {explorerTakeoverEnabled
                              ? 'Takeover actif pendant la session.'
                              : 'Windows reste natif.'}
                          </span>
                          <span className={cn(
                      ...
                      <div className="space-y-2">
                        ...
                        <Slider
                          ...
                          onValueChange={(value) => {
                            onSurfaceOpacityChange(Math.min(0.95, Math.max(0.5, value[0] / 100)));
                          }}
                        />
                      </div>
